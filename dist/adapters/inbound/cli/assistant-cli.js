...
const TOOL_OPTIONS = [
    ...
    },
    {
        ...
        audiences: ['customer', 'credit_specialist'],
...
function printOptions(audience) {
    console.log(`\nPerfil ativo: ${audience === 'customer' ? 'cliente' : 'especialista'}`);
    console.log('Opções disponíveis:');
    TOOL_OPTIONS.filter((option) => option.audiences.includes(audience)).forEach((option) => console.log(` ${option.id} - ${option.description}`));
    console.log('');
}
function getSelectedOption(selection, audience) {
...
async function bootstrap() {
    ...
    const assistant = appContext.get('AssistantWorkflowPort');
    ...
    rl.on('line', async (line) => {
        ...
        const request = { userMessage: message, audience: sessionState.audience };
        try {
